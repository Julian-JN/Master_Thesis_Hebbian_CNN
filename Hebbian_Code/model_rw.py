...
class Net_RW(nn.Module):
    def __init__(self, rw_params=None):
        super(Net_RW, self).__init__()

        # A single convolutional layer
        self.bn1 = nn.BatchNorm2d(3, affine=False)
        self.conv1 = RWConv2d(in_channels=3, out_channels=96, kernel_size=5, stride=1, padding=0)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.activ1 = Triangle(power=1.)

        self.bn2 = nn.BatchNorm2d(96, affine=False)
        self.conv2 = RWConv2d(in_channels=96, out_channels=384, kernel_size=3, stride=1, padding=0)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.activ2 = Triangle(power=1.)

        self.bn3 = nn.BatchNorm2d(384, affine=False)
        self.conv3 = RWConv2d(in_channels=384, out_channels=1536, kernel_size=3, stride=1, padding=0)
